...
export function Dashboard() {
  ...
  const resetAllEditStates = () => {
    ...
    setCategoryForm({ name: "", slug: "", tagline: "", image: "" });
  ...
  const loadAllData = async () => {
    ...
    try {
      if (token) {
        try {
          const prodData = await productsApi.getAll({ limit: 10000 });
  ...
  const saveProduct = async (e) => {
    ...
    formData.append("featured", (productForm.tag === "Featured" || productForm.tag === "Top Seller") ? "true" : "false");
  ...
  const handleSaveCategory = async (e) => {
    ...
    try {
      if (editingCategory) {
        await categoriesApi.update(editingCategory._id, formData, token);
      ...
      toast.error(err?.message || `Failed to ${editingCategory ? "update" : "add"} category`);
    }
  };

  const deleteCategory = async (id, slug) => {
    if (!id) {
      toast.error("Cannot delete static fallback data.");
      return;
    }
  ...
  const handleSaveBrand = async (e) => {
    ...
    try {
      if (editingBrand) {
        await brandsApi.update(editingBrand._id, formData, token);
      ...
      toast.error(err?.message || `Failed to ${editingBrand ? "update" : "add"} brand`);
    }
  };

  const deleteBrand = async (id, slug) => {
    if (!id) {
      toast.error("Cannot delete static fallback data.");
      return;
    }
    if (confirm("Are you sure you want to delete this brand?")) {
      try {
        await brandsApi.delete(id, token);
      ...
      <aside className="w-full md:w-64 bg-card border-r border-border flex flex-col shrink-0">
        ...
        <nav className="flex-1 p-4 space-y-1">
          <TabButton
            ...
            onClick={() => { setActiveTab("products"); resetAllEditStates(); }}
            icon={Package}
            label="Products"
          />
          <TabButton
            active={activeTab === "categories"}
            onClick={() => { setActiveTab("categories"); resetAllEditStates(); }}
            icon={FolderOpen}
            label="Categories"
          />
          <TabButton
            active={activeTab === "brands"}
            onClick={() => { setActiveTab("brands"); resetAllEditStates(); }}
            icon={Tag}
            label="Brands"
          />
          <TabButton
            active={activeTab === "enquiries"}
            onClick={() => { setActiveTab("enquiries"); resetAllEditStates(); }}
            icon={MessageSquare}
            label="Enquiries"
          ...
          <TabButton
            ...
            onClick={() => { setActiveTab("schedules"); resetAllEditStates(); }}
            icon={CalendarDays}
            label="Schedules"
          />
          <TabButton
            active={activeTab === "appointments"}
            onClick={() => { setActiveTab("appointments"); resetAllEditStates(); }}
            icon={CalendarClock}
            label="Appointments"
                        ...
                        <p className="text-sm text-muted-foreground mt-0.5">
                          {productSearch
                            ? `${products.filter(p => {
                              ...
                              const brandName = typeof p.brand === "object" ? p.brand?.name : (brands.find(b => b.slug === p.brand || b._id === p.brand)?.name || p.brand || "");
                              const catName = p.category?.name || categories.find(c => c.slug === p.categorySlug || c._id === p.category)?.name || p.categorySlug || "";
                              return p.name?.toLowerCase().includes(q) || brandName?.toLowerCase().includes(q) || catName?.toLowerCase().includes(q);
                      ...
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <div className="relative">
                          ...
                          <input
                            ...
                            className="h-10 w-64 rounded-xl border border-border bg-background pl-9 pr-3 text-sm outline-none focus:border-primary transition"
                          />
                          {productSearch && (
                            <button
                              onClick={() => setProductSearch("")}
                              className="absolute right-2.5 top-1/2 -translate-y-1/2 p-0.5 text-muted-foreground hover:text-foreground transition"
                            >
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-2 bg-primary text-primary-foreground font-bold rounded-full px-5 py-2.5 text-sm transition hover:opacity-95 active:scale-95 shadow-soft"
                        >
                        ...
                        <tbody>
                          {products
                            .filter((p) => {
                              ...
                              const brandName = typeof p.brand === "object"
                                ? p.brand?.name
                                : (brands.find((b) => b.slug === p.brand || b._id === p.brand)?.name || p.brand || "");
                              const catName = p.category?.name ||
                                categories.find((c) => c.slug === p.categorySlug || c._id === p.category)?.name ||
                                p.categorySlug || "";
                              return (
                          ...
                          {products.filter((p) => {
                            ...
                            return (
                              ...
                              <tr>
                                <td colSpan={7} className="px-6 py-10 text-center text-muted-foreground font-medium text-sm">
                    ...
                    <form onSubmit={saveProduct} className="space-y-4">
                      <div className="grid gap-4 sm:grid-cols-2">
                        ...
                        <div className="space-y-1">
                          ...
                          <select required value={productForm.categorySlug} onChange={(e) => setProductForm({ ...productForm, categorySlug: e.target.value })} className="h-11 w-full rounded-xl border border-border bg-background px-3.5 text-sm outline-none focus:border-primary">
                        ...
                        <div className="space-y-1">
                          ...
                          <select required value={productForm.brand} onChange={(e) => setProductForm({ ...productForm, brand: e.target.value })} className="h-11 w-full rounded-xl border border-border bg-background px-3.5 text-sm outline-none focus:border-primary">
                      ...
                      <div className="flex justify-end gap-3 border-t border-border pt-4">
                        ...
                        <button type="submit" className="rounded-xl bg-primary text-primary-foreground px-5 py-2.5 text-sm font-bold hover:opacity-95 transition active:scale-95 shadow-soft">Save Product</button>
                      </div>
                    </form>
                  </div>
                )}
              ...
              <div className="space-y-6">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-0.5">
                      {categorySearch
                        ? `${categories.filter(c => c.name?.toLowerCase().includes(categorySearch.toLowerCase()) || c.slug?.toLowerCase().includes(categorySearch.toLowerCase())).length} of ${categories.length} categories`
                        : "Manage smart home product categories."}
                    </p>
                  </div>
                  <div className="flex items-center gap-3 flex-wrap">
                    <div className="relative">
                ...
                <div className="flex gap-4">
                  <div className="bg-card border border-border rounded-2xl px-5 py-3 shadow-soft flex items-center gap-3">
                    <div className="rounded-xl bg-primary/10 p-2 text-primary">
                      <FolderOpen className="h-5 w-5" />
                    </div>
                    <div>
                      <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Total Categories</p>
                      <h3 className="text-xl font-extrabold text-foreground">{categories.length}</h3>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid gap-6 md:grid-cols-[1.5fr_1fr]">
                      ...
                      <tbody>
                            ...
                            <tr key={c.slug} className="border-b border-border last:border-0 hover:bg-secondary/10 transition-colors">
                              ...
                              <td className="px-6 py-3 text-right">
                                <div className="inline-flex items-center gap-1.5 justify-end w-full">
                                  <button onClick={() => startEditCategory(c)} className="p-2 text-foreground/80 hover:text-foreground hover:bg-secondary rounded-xl transition" title="Edit">
                                    <Edit2 className="h-4 w-4" />
                                  </button>
                                  <button onClick={() => deleteCategory(c._id, c.slug)} className="p-2 text-destructive hover:bg-destructive/10 rounded-xl transition" title="Delete">
                                    <Trash2 className="h-4 w-4" />
                                  </button>
                                </div>
                              </td>
                        ...
                        {categories.filter((c) => {
                          ...
                          return (
                            ...
                            <tr>
                              <td colSpan={4} className="px-6 py-10 text-center text-muted-foreground font-medium text-sm">
                                No categories found matching &ldquo;{categorySearch}&rdquo;
                              </td>
                            </tr>
                          )}
                      </tbody>
                  ...
                  <div className="bg-card border border-border rounded-2xl p-6 shadow-card h-fit">
                    <h3 className="font-bold text-base mb-4">{editingCategory ? "Edit Category" : "Add Category"}</h3>
                    <form onSubmit={handleSaveCategory} className="space-y-4">
                      ...
                      <div className="space-y-1">
                        ...
                        <input value={categoryForm.image} onChange={(e) => setCategoryForm({ ...categoryForm, image: e.target.value })} placeholder="e.g. /src/assets/cat-cctv.jpg" className="h-10 w-full rounded-xl border border-border bg-background px-3 text-sm outline-none focus:border-primary" />
                      </div>
                      <div className="flex gap-2.5">
                        {editingCategory && (
                          <button type="button" onClick={cancelEditCategory} className="w-1/2 h-10 rounded-xl border border-border bg-card font-semibold text-sm hover:bg-secondary transition active:scale-95">Cancel</button>
                        )}
                        <button type="submit" className={`h-10 rounded-xl bg-primary text-primary-foreground font-bold text-sm hover:opacity-95 active:scale-95 transition shadow-soft ${editingCategory ? "w-1/2" : "w-full"}`}>
              ...
              <div className="space-y-6">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-0.5">
                      {brandSearch
                        ? `${brands.filter(b => b.name?.toLowerCase().includes(brandSearch.toLowerCase()) || b.slug?.toLowerCase().includes(brandSearch.toLowerCase())).length} of ${brands.length} brands`
                        : "Manage partner smart home automation brands."}
                    </p>
                  </div>
                  <div className="flex items-center gap-3 flex-wrap">
                    <div className="relative">
                ...
                <div className="flex gap-4">
                  <div className="bg-card border border-border rounded-2xl px-5 py-3 shadow-soft flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Total Brands</p>
                      <h3 className="text-xl font-extrabold text-foreground">{brands.length}</h3>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid gap-6 md:grid-cols-[1.5fr_1fr]">
                  <div className="border border-border rounded-2xl bg-card overflow-hidden shadow-card">
                    <table className="w-full text-left border-collapse text-sm">
                      ...
                      <tbody>
                        {brands
                          ...
                          .map((b) => (
                            <tr key={b.slug} className="border-b border-border last:border-0 hover:bg-secondary/10 transition-colors">
                              ...
                              <td className="px-6 py-3 text-right">
                                <div className="inline-flex items-center gap-1.5 justify-end w-full">
                                  <button onClick={() => startEditBrand(b)} className="p-2 text-foreground/80 hover:text-foreground hover:bg-secondary rounded-xl transition" title="Edit">
                                    <Edit2 className="h-4 w-4" />
                                  </button>
                                  <button onClick={() => deleteBrand(b._id, b.slug)} className="p-2 text-destructive hover:bg-destructive/10 rounded-xl transition" title="Delete">
                                    <Trash2 className="h-4 w-4" />
                                  </button>
                                </div>
                              </td>
                        ...
                        {brands.filter((b) => {
                          if (!brandSearch) return false;
                          const q = brandSearch.toLowerCase();
                          return b.name?.toLowerCase().includes(q) || b.slug?.toLowerCase().includes(q);
                        }).length === 0 && brandSearch && (
                            <tr>
                              <td colSpan={3} className="px-6 py-10 text-center text-muted-foreground font-medium text-sm">
                  ...
                  <div className="bg-card border border-border rounded-2xl p-6 shadow-card h-fit">
                    <h3 className="font-bold text-base mb-4">{editingBrand ? "Edit Brand" : "Add Brand"}</h3>
                    <form onSubmit={handleSaveBrand} className="space-y-4">
                      ...
                      <div className="space-y-1">
                        ...
                        <input value={brandForm.logo} onChange={(e) => setBrandForm({ ...brandForm, logo: e.target.value })} placeholder="e.g. Logo image path or link" className="h-10 w-full rounded-xl border border-border bg-background px-3 text-sm outline-none focus:border-primary" />
                      </div>
                      <div className="flex gap-2.5">
                        {editingBrand && (
                          <button type="button" onClick={cancelEditBrand} className="w-1/2 h-10 rounded-xl border border-border bg-card font-semibold text-sm hover:bg-secondary transition active:scale-95">Cancel</button>
                        )}
                        <button type="submit" className={`h-10 rounded-xl bg-primary text-primary-foreground font-bold text-sm hover:opacity-95 active:scale-95 transition shadow-soft ${editingBrand ? "w-1/2" : "w-full"}`}>
              ...
              <div className="space-y-6">
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-0.5">
                      {enquirySearch
                        ? `${enquiries.filter(e => e.name?.toLowerCase().includes(enquirySearch.toLowerCase()) || e.phone?.toLowerCase().includes(enquirySearch.toLowerCase()) || e.email?.toLowerCase().includes(enquirySearch.toLowerCase()) || e.category?.toLowerCase().includes(enquirySearch.toLowerCase()) || e.message?.toLowerCase().includes(enquirySearch.toLowerCase())).length} of ${enquiries.length} enquiries`
                        : "Review lead forms and WhatsApp callback requests."}
                    </p>
                  </div>
                  <div className="flex items-center gap-3 flex-wrap">
                    <div className="relative">
                ...
                <div className="border border-border rounded-2xl bg-card overflow-hidden shadow-card">
                  <table className="w-full text-left border-collapse text-sm">
                    ...
                    <tbody>
                          ...
                          <tr key={e._id} className="border-b border-border last:border-0 hover:bg-secondary/10 transition-colors">
                            ...
                            <td className="px-6 py-3.5">
                              <select
                                ...
                                className={`rounded-full px-2.5 py-1 text-xs font-semibold outline-none border border-border ${e.status === "Completed"
                      ...
                      }).length === 0 && enquirySearch && (
                          <tr>
                            <td colSpan={6} className="px-6 py-12 text-center text-muted-foreground font-medium">
                              No enquiries found matching &ldquo;{enquirySearch}&rdquo;
                            </td>
                          </tr>
                        )}
                      {enquiries.length === 0 && !enquirySearch && (
                        <tr>
      ...
      <div className="rounded-2xl border border-border bg-card p-6 mb-8 shadow-sm">
        ...
        <form onSubmit={handleAdd} className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5 items-end">
          ...
          <button
            type="submit"
            disabled={saving}
            className="flex h-10 items-center justify-center gap-2 rounded-xl bg-primary px-5 text-sm font-bold text-primary-foreground hover:opacity-95 transition active:scale-95 disabled:opacity-60 shadow-soft"
          >
        ...
        <div className="flex gap-2 flex-wrap">
          {["all", "Pending", "Confirmed", "Completed", "Cancelled"].map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`rounded-full px-4 py-1.5 text-xs font-bold transition capitalize ${filter === f ? "bg-primary text-primary-foreground shadow-soft" : "bg-secondary text-foreground hover:bg-secondary/70"}`}
            >
              {f}
            </button>
          ))}
...
function TabButton({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex w-full items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition ${active
